...
def test_select_greedy(
    lsys, decode_config, device, exec_req_list, multi_greedy_token_selection_strategy
):
    async def _test():
        count = 0
        ...
        selections = await multi_greedy_token_selection_strategy.select_greedy(
            beams, []
        )
        ...
        assert [
            selection.last_token for selection in selections
        ] == expected_last_tokens

    lsys.run(_test())


def test_multi_greedy_decode_single(
    lsys,
    cache,
    device,
    dummy_pages,
